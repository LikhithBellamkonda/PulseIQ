# train_model.py
"""
2. MODEL TRAINING & CALIBRATION PIPELINE
Loads the synthetic stress dataset, performs a stratified 70/15/15 train-val-test split, 
trains Random Forest and Gradient Boosting Classifiers, compares performance, selects 
the best model automatically, saves the selected model, and calibrates regression-based 
feature thresholds into a lightweight JSON configuration for real-time web dashboard inference.
"""
...
data_path = os.path.join(ml_dir, "synthetic_stress_dataset.csv")

if not os.path.exists(data_path):
    raise FileNotFoundError(f"Dataset CSV not found at {data_path}. Run synthetic_dataset.py first.")

df = pd.read_csv(data_path)

# Features: Heart Rate and Ambient Light Intensity
X = df[["heart_rate", "light"]]
# Classification targets
y_class = df["stress_category"]
# Regression targets for threshold/index calibration
y_score = df["stress_score"]

# 2. Split the dataset (70% Train, 15% Validation, 15% Test)
# Stratify on class labels to maintain perfect balance across splits
...
print("[INFO] Data split completed successfully:")
print(f"   - Training set size:   {X_train.shape[0]} samples")
print(f"   - Validation set size: {X_val.shape[0]} samples")
print(f"   - Test set size:       {X_test.shape[0]} samples\n")

# 3. Train Candidate Classifier Models
candidates = {
...
print(f"\n[SUCCESS] Selected Best Model: {best_model_name} with Accuracy: {best_acc * 100:.2f}%")

# 4. Evaluate Selected Model on Hold-Out Test Set
...
# 5. Serialize the Best Classifier Model
model_path = os.path.join(ml_dir, "best_model.pkl")
joblib.dump(best_model, model_path)
print(f"\n[SAVE] Saved classifier model binary to: {model_path}")

# 6. Calibrate Regression-Based Score Thresholds for Dashboard Runtime
# Train an explainable, lightweight Linear Regression model to map (HR, Light) -> Stress Score.
# This mathematically distills the decision space into a closed-form formula:
# Stress Score = Intercept + (w_hr * heart_rate) + (w_light * light)
...
# Calculate predicted stress scores on training sets to find optimal class boundaries
...
# Calculate mean predicted scores for each category
class_means = df_train.groupby("true_category")["pred_score"].mean().to_dict()

# Calculate decision boundary midpoints between successive categories
...
# Structure and save parameters to JSON config
thresholds_config = {
    "model_info": {
        "description": "Linear regression weights derived from Random Forest selection to map HR and Light to a continuous 0-100 Stress Index.",
...
thresholds_path = os.path.join(ml_dir, "thresholds.json")
with open(thresholds_path, "w") as f:
    json.dump(thresholds_config, f, indent=4)

print(f"[SAVE] Saved thresholds and coefficients configuration to: {thresholds_path}")
